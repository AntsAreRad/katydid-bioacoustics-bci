...
#' 
#' (1) Simple format: columns 'species' and 'confidence_threshold' (0-1).
#' (2) Doug Robinson format: columns 'species', 'confidence_threshold',
#'     'exclude', 'threshold_99', 'threshold_95', 'threshold_90'.
#'     Generated by convert_doug_robinson_thresholds.R.
#'
#' @param filepath Character. Path to CSV file with species thresholds.
#' @param threshold_level Character. Which precision level to use when
#'   multi-level thresholds are present: "99", "95", or "90". Default: "99".
#'   Fallback: if the chosen level is NA for a species, falls back to the
#'   next stricter level (90 -> 95 -> 99).
#'
#' @return Data frame with at least 'species' and 'confidence_threshold' columns,
#'   or NULL if the file cannot be loaded.
#'   Excluded species have confidence_threshold = Inf.
#'   Species with "all correct" evaluation have confidence_threshold = 0.
#'
#' @details
#' Species not listed in the CSV will use the default uniform threshold
#' set in \code{CONFIG$bird_confidence} (applied by apply_species_thresholds).
#'
#' @examples
#' \dontrun{
#' # Load Doug Robinson thresholds at 95% precision
#' thresholds <- load_species_thresholds(
#'   "data/bird_species_thresholds_doug_robinson.csv",
#'   threshold_level = "95"
#' )
#' }
#'
...
load_species_thresholds <- function(filepath, threshold_level = "99") {
  ...
  tryCatch({
    ...
    # --- Handle multi-level thresholds (Doug Robinson format) ---
    # If the file has threshold_99/95/90 columns, select the right one
    # and populate 'confidence_threshold' accordingly.
    has_multi_level <- all(c("threshold_99", "threshold_95", "threshold_90") 
                           %in% colnames(thresholds))
    
    if (has_multi_level && threshold_level %in% c("99", "95", "90")) {
      cat(sprintf("    Multi-level thresholds detected, using %s%% precision level\n",
                  threshold_level))
      
      # Select threshold with fallback to more permissive levels
      # e.g. if level=99 but only threshold_95 exists, use threshold_95
      thresholds <- thresholds %>%
        mutate(
          confidence_threshold = case_when(
            threshold_level == "90" & !is.na(threshold_90) ~ threshold_90,
            threshold_level == "90" & !is.na(threshold_95) ~ threshold_95,
            threshold_level == "90" & !is.na(threshold_99) ~ threshold_99,
            threshold_level == "95" & !is.na(threshold_95) ~ threshold_95,
            threshold_level == "95" & !is.na(threshold_99) ~ threshold_99,
            threshold_level == "99" & !is.na(threshold_99) ~ threshold_99,
            threshold_level == "99" & !is.na(threshold_95) ~ threshold_95,
            threshold_level == "99" & !is.na(threshold_90) ~ threshold_90,
            TRUE ~ confidence_threshold  # keep existing value as last resort
          )
        )
    }
    ...
    # Coerce threshold values
    thresholds$confidence_threshold <- as.numeric(thresholds$confidence_threshold)
    
    # --- Handle excluded species (Doug Robinson format) ---
    # Species with exclude=TRUE are kept in the table but marked for removal.
    # apply_species_thresholds() will filter them out.
    has_exclude <- "exclude" %in% colnames(thresholds)
    
    if (has_exclude) {
      thresholds$exclude <- as.logical(thresholds$exclude)
      n_excluded <- sum(thresholds$exclude, na.rm = TRUE)
      cat(sprintf("    [!] %d species flagged for EXCLUSION (all detections are false positives)\n",
                  n_excluded))
      # For excluded species, set threshold to Inf so they are filtered out
      thresholds <- thresholds %>%
        mutate(confidence_threshold = ifelse(exclude, Inf, confidence_threshold))
    }
    
    # Validate threshold values for non-excluded species
    valid_thresholds <- thresholds %>%
      filter(!is.infinite(confidence_threshold))
    
    invalid_rows <- is.na(valid_thresholds$confidence_threshold) | 
      valid_thresholds$confidence_threshold < 0 | 
      valid_thresholds$confidence_threshold > 1
    
    if (any(invalid_rows)) {
      n_invalid <- sum(invalid_rows)
      warning(sprintf("%d rows with invalid thresholds (NA or outside 0-1) - these species will use the default threshold", 
                      n_invalid))
      # Remove rows with invalid thresholds (they'll fall back to default)
      invalid_species <- valid_thresholds$species[invalid_rows]
      thresholds <- thresholds %>%
        filter(!(species %in% invalid_species & !is.infinite(confidence_threshold)))
    }
    ...
    # Report summary
    n_custom    <- sum(is.finite(thresholds$confidence_threshold) & 
                        thresholds$confidence_threshold > 0)
    n_all_ok    <- sum(thresholds$confidence_threshold == 0, na.rm = TRUE)
    n_excluded  <- sum(is.infinite(thresholds$confidence_threshold))
    
    cat(sprintf("    [OK] Loaded thresholds for %d species\n", nrow(thresholds)))
    cat(sprintf("      - Custom threshold: %d species\n", n_custom))
    cat(sprintf("      - All correct (threshold=0): %d species\n", n_all_ok))
    cat(sprintf("      - Excluded (false positives): %d species\n", n_excluded))
    
    # Show threshold range for valid species
    finite_thresholds <- thresholds$confidence_threshold[
      is.finite(thresholds$confidence_threshold) & thresholds$confidence_threshold > 0
    ]
    if (length(finite_thresholds) > 0) {
      cat(sprintf("      - Threshold range: %.2f - %.2f (median: %.2f)\n",
                  min(finite_thresholds), max(finite_thresholds), 
                  median(finite_thresholds)))
    }
...
apply_species_thresholds <- function(detections,
  ...
  # Remove BirdNET "nocall" entries (no detection, not a real species)
  n_nocall <- sum(detections[[species_col]] == "nocall", na.rm = TRUE)
  if (n_nocall > 0) {
    detections <- detections %>% filter(.data[[species_col]] != "nocall")
    cat(sprintf("    [--] Removed %d 'nocall' entries\n", n_nocall))
  }
  ...
  # Build a lookup: species -> threshold (Inf = excluded, 0 = keep all)
  ...
  # Log exclusion/threshold stats
  n_before <- nrow(detections)
  n_excluded_detections <- sum(is.infinite(detections$threshold_applied))
  n_custom <- sum(is.finite(detections$threshold_applied) & 
                    detections$threshold_applied != default_threshold)
  n_default <- sum(detections$threshold_applied == default_threshold)
  
  excluded_species <- unique(detections[[species_col]][is.infinite(detections$threshold_applied)])
  if (length(excluded_species) > 0) {
    cat(sprintf("    [!] Excluding %d species (all false positives per Doug Robinson): %d detections removed\n",
                length(excluded_species), n_excluded_detections))
  }
  
  # Filter each detection against its own threshold
  # Inf threshold means the species is excluded (confidence can never be >= Inf)
  filtered <- detections %>%
    filter(confidence >= threshold_applied)
  
  cat(sprintf("    Species-specific thresholds applied: %d -> %d detections\n",
              n_before, nrow(filtered)))
  cat(sprintf("      Custom threshold: %d detections, Default (%.2f): %d detections\n",
              sum(filtered$threshold_applied != default_threshold & 
                    is.finite(filtered$threshold_applied)),
              default_threshold,
              sum(filtered$threshold_applied == default_threshold)))
          ...
          if (file.exists(file_path) && file.size(file_path) > 0) {
            ...
            if (nrow(bird_data) > 0) {
              ...
              if (all(required_cols %in% names(bird_data))) {
                names(bird_data)[names(bird_data) == "Species Code"] <- "species_code"
                names(bird_data)[names(bird_data) == "Common Name"] <- "common_name"
                names(bird_data)[names(bird_data) == "Confidence"] <- "confidence"
